...
if ( ! defined( 'AGENCYPRO_VERSION' ) ) {
	define( 'AGENCYPRO_VERSION', '2.2.1' );
}
...
function agencypro_widgets_init() {
	register_sidebar(['name' => esc_html__( 'Footer', 'agencypro' ), 'id' => 'footer-1', 'before_widget' => '<section id="%1$s" class="widget %2$s">', 'after_widget'  => '</section>', 'before_title'  => '<h2 class="widget-title">', 'after_title'   => '</h2>']);
	register_sidebar(['name' => esc_html__( 'Blog Sidebar', 'agencypro' ), 'id' => 'blog-sidebar', 'before_widget' => '<section id="%1$s" class="widget %2$s">', 'after_widget'  => '</section>', 'before_title'  => '<h2 class="widget-title">', 'after_title'   => '</h2>']);
}
...
function agencypro_scripts() {
    ...
    wp_localize_script( 'agencypro-main-js', 'agencypro_ajax_obj', ['ajax_url' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'agencypro_filter_nonce' )]);
}
...
function agencypro_dynamic_css() {
    ...
    foreach ($sections as $section) {
        ...
        switch ($bg_type) {
            case 'color': $css .= "{$selector} { background: " . esc_attr(get_theme_mod("agencypro_{$section}_bg_color")) . "; }"; break;
            case 'image': $css .= "{$selector} { background-image: url(" . esc_url(get_theme_mod("agencypro_{$section}_bg_image")) . "); background-size: cover; background-position: center; }"; break;
            case 'gradient': $css .= "{$selector} { background-image: linear-gradient(to right, " . esc_attr(get_theme_mod("agencypro_{$section}_bg_gradient_1")) . ", " . esc_attr(get_theme_mod("agencypro_{$section}_bg_gradient_2")) . "); }"; break;
            default: $css .= "{$selector} { background: none !important; }"; break;
        }
    }
    if (!empty($css)) { wp_add_inline_style('agencypro-main-style', $css); }
}
...
function agencypro_filter_portfolio() {
    ...
    if ($term_slug !== 'all') { $args['tax_query'] = [['taxonomy' => 'service_type', 'field' => 'slug', 'terms' => $term_slug]]; }
    $query = new WP_Query($args);
    ob_start();
    if ($query->have_posts()) {
        while ($query->have_posts()) { $query->the_post(); get_template_part('template-parts/content', 'project'); }
    } elseif ($page === 1) { echo '<p class="no-results">' . esc_html__( 'No projects found.', 'agencypro' ) . '</p>'; }
    $html = ob_get_clean();
    wp_reset_postdata();
